...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Nested function can access the internal variable because the closue allows anything inside of it to reach outside of it's scope 
//all the way to global scope/
...
function summation(num) {
  //Oh my gosh this was the hardest one. It would have been so much easier to recieve an array than ONE number. Jesus...
  let counter = 0 
  for(let i = 0; i <= num; i++){
    counter += i
  }
...
const zooAnimals = [
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const newArr = arr.filter(x => x.population < 5)
    return newArr
  }
...
function childCube(childattr){
  CuboidMaker.call(this, childattr)
  this.isChild = childattr.isChild
}

childCube.prototype = Object.create(CuboidMaker.prototype)

const cuboid = new childCube({
  length: 4,
  width: 5,
  height: 5,
})
